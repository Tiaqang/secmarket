        ...
        viewPager.addOnPageChangeListener(new ViewPager.SimpleOnPageChangeListener() {
            ...
            public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                ...
                if (mIndicatorMode != Mode.SOLO) {
                    ...
                    Log.d("flag", "--------------------->CircleIndicator onPageScrolled");
                }
            }
...
//                记录已经选中的位置，如果有所滑动该位置就相当于旧位置
                currentPosition = position;
                if (mIndicatorMode == Mode.SOLO) {
                    Log.d("flag", "--------------------->CircleIndicator mIndicator: solo");
                    trigger(position, 0);
                }
            }
    ...
    private void trigger(int position, float positionOffset) {
        lastPosition = position;//当前的选中位置
        position = position % count;
        if (position == count - 1) {
            if (lastPosition < currentPosition) {
                CircleIndicator.this.mCurItemPosition = 0;
            } else {
                CircleIndicator.this.mCurItemPosition = position;
            }
            CircleIndicator.this.mCurItemPositionOffset = 0;
